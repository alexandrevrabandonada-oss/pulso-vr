...
PUBLISHED_YEARS = tuple(range(2010, 2023)) + (2024,)
...
def build_neurological_rates(root: Path) -> tuple[Path, Path, Path]:
    if set(_definitions(root)) != set(OUTCOMES):
        raise ValueError("neurological outcome configuration is incomplete")
    paths = sorted(
        path for path in (root / "data" / "interim").glob("sim_*_harmonized.parquet")
        if _year(path) in PUBLISHED_YEARS
    )
    if not paths:
    ...
    query = f"""
        WITH base AS (
            ...
            WHERE CAST(source_year AS INTEGER) IN (2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2024)
        ), mapped AS (
            SELECT
                b.year,
    ...
    if observed_years != list(PUBLISHED_YEARS):
        raise ValueError(f"expected neurological SIM years {list(PUBLISHED_YEARS)}, found {observed_years}")
    ...
    report_path.write_text(
        ...
        "2023 permanece como lacuna real e não é interpolado; 2024 é o dado recente secundário.\n",
        encoding="utf-8",
    )
