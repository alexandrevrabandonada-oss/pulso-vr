...
export default async function handler(request: Request) {
  try {
    ...
    const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escape(title)}</title><meta name="description" content="${escape(description)}"><meta property="og:type" content="website"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:image" content="${escape(image)}"><meta property="og:url" content="${escape(url.toString())}"><meta name="twitter:card" content="summary_large_image"><link rel="canonical" href="${escape(model.canonicalUrl)}"></head><body><main><h1>${escape(title)}</h1><p>${escape(description)}</p><p><a href="${escape(model.canonicalUrl)}">Abrir dado no Observatório</a></p></main><script>setTimeout(function(){location.replace(${JSON.stringify(model.canonicalUrl)})},600)</script><noscript><a href="${escape(model.canonicalUrl)}">Continuar para o Observatório</a></noscript></body></html>`
