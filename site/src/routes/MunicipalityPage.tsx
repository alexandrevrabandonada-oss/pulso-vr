...
export function MunicipalityPage() {
  ...
  const municipality = municipalities.find((item) => item.code === params?.codigo)
  const requestedIndicator = new URLSearchParams(search).get('indicador')
  const indicator = catalog.indicators.find((item) => item.id === requestedIndicator)
    ?? catalog.indicators.find((item) => item.id === 'sih-pneumonia') ?? catalog.indicators[0]
  ...
  useEffect(() => {
    ...
    Promise.all([loadSeries(indicator.id), loadMunicipalSeries(indicator.id), loadMap(indicator.id)])
      .then(([seriesPayload, municipalPayload, mapPayload]) => {
        if (active) { setSeries(seriesPayload.observations); setMunicipalSeries(municipalPayload.observations); setMap(mapPayload) }
  ...
  if (!series || !municipalSeries || !map) return <main className="content-page"><LoadingState label={`Carregando dados de ${municipality.name}…`} /></main>
  ...
  return (
    <main className="municipality-page">
      ...
      <section className="municipality-visuals">
        ...
        <TerritoryMap topology={topology} values={map.values} scaleDomain={map.mapScale?.domain} status={map.status} period={map.period} selectedCode={municipality.code} onSelect={(code) => navigate(`/municipios/${code}?indicador=${indicator.id}`)} />
      </section>
      <section className="municipality-method"><FileText /><div><h2>Como interpretar</h2><p>{indicator.allowsConclusion} {indicator.doesNotAllowConclusion}</p><p><strong>Perfis por idade e sexo:</strong> {indicator.profileCoverage?.status === 'pilot' ? 'piloto agregado disponível; expansão municipal em validação.' : 'indisponíveis nesta release.'}</p><Link href={`/indicadores/${indicator.id}`}>Ver fonte, CID e limitações</Link></div></section>
    </main>
  )
}
