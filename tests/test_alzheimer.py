...
def test_generated_sim_neurological_series_covers_all_rj_municipalities() -> None:
    ...
    assert 2023 not in municipal["year"].unique()


def test_standardized_rate_is_published_only_for_2022_and_small_cells_are_hidden() -> None:
    import json

    payload = json.loads((ROOT / "site" / "public" / "data" / "municipal-series" / "sim-alzheimer.json").read_text(encoding="utf-8"))
    standardized = [row for row in payload["observations"] if row["metricKind"] == "age_sex_standardized_rate_per_100k"]
    assert {row["period"] for row in standardized} == {"2022"}
    assert len({row["geographyId"] for row in standardized}) == 92
    for row in standardized:
        if row["suppressed"]:
            assert row["value"] is None
            assert row["count"] is None
            assert row["ciLow"] is None
            assert row["ciHigh"] is None
